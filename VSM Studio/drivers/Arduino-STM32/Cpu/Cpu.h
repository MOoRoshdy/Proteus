...
class CPU
 { public:
      // Thunks to Arduino layer:
 	   static void pinMode(uint8_t pin, WiringPinMode mode) { ::pinMode(pin, mode); }
	   ...
	   static void randomSeed(long seed) { return ::randomSeed(seed); }
 	   ...
 	   // Interrupt hooks
 	   static void enableInterrupt(uint8_t i, ExtIntTriggerMode mode) { ::attachInterrupt(i, isrs[i], mode); }
      ...
      //Built-in LED
	static void setBuiltInLED(bool s) { pinMode(LED_BUILTIN, OUTPUT); digitalWrite(LED_BUILTIN, s ? LOW : HIGH); }
